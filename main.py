    ...
    return AvailabilityResponse(
        ...
        slots=[
            AvailabilitySlot(
                ...
                maxDuration=150,
                metaData=BookingTimeSlotMetaData(sevenRooms={"type": "request", "access_persistent_id": "example-id"}),
                dobRequired=False,
...
async def get_pre_confirmation(
    ...
    availability_slot: AvailabilitySlot = Body(..., description="The selected availability slot"),
) -> PreConfirmationResponse:
    return PreConfirmationResponse(
        notes="We have a 5 minute grace period. Please call us if you are running later.",
        bookingUrl=(availability_slot.url if availability_slot.url else "https://booking.com/my-booking-form"),
    )
...
async def list_bookings(
    ...
    upcoming_only: bool = Query(False, description="If true, only returns future bookings"),
    limit: int = Query(50, ge=1, le=100, description="Maximum number of bookings to return"),
) -> list[Booking]:
